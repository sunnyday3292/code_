...
pair<ll, pair<pll,pll>> rotatingCalipers(const vector<pll>& hull){
    int m = hull.size();
    if(m == 1) return {0,{hull[0],hull[0]}};
    if(m == 2) return {dist2(hull[0], hull[1]), {hull[0], hull[1]}};
    
    ll ans = 0;
    pll A = hull[0], B = hull[0];
    int j = 1;
    for(int i=0;i<m;i++){
        int ni = (i+1)%m;
        while(true){
...
int main() {
    ...
    while(T--){
        ...
        auto res = rotatingCalipers(inp);
        pll A = res.second.first;
        pll B = res.second.second;
